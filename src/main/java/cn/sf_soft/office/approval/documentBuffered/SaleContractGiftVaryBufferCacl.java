 ...
 *
 */
@Service("saleContractGiftVaryBufferCacl")
public class SaleContractGiftVaryBufferCacl extends DocumentBufferCalc {
	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(SaleContractGiftVaryBufferCacl.class);
	private static String bufferVersion = "20161128.0";
	private static String bufferClass = SaleContractGiftVaryBufferCacl.class.getSimpleName();
	...
	private List<VehicleSaleContractGifts> getNoChangeList(VehicleSaleContractDetailVary contractDetailVary) {
		String hql = "from VehicleSaleContractGifts as a where not exists (select 1 from VehicleSaleContractGiftsVary as b  where a.detailId = b.detailId and b.contractDetailId = ? and  b.detailVaryId = ?) and a.contractDetailId = ?";
		List<VehicleSaleContractGifts> list = (List<VehicleSaleContractGifts>) dao.findByHql(hql,
	...
	public MobileBoardBuffered computeFields(VehicleSaleContractDetailVary contractDetailVary, String vehicleTitle) {
		...
		List<VehicleSaleContractGifts> noChangeList = getNoChangeList(contractDetailVary);
		if ((giftVaryList == null || giftVaryList.size() == 0) && (noChangeList == null || noChangeList.size() == 0)) {
			return null;
		}
		...
		short sn = 1;
		int n = 0;
		for (int i = 0; i < giftVaryList.size(); i++) {
			VehicleSaleContractGiftsVary gift = giftVaryList.get(i);
			sn = createDetailField(sn, n, summaryBoard, gift);
			n++;
		}

		for (int i = 0; i < noChangeList.size(); i++) {
			VehicleSaleContractGifts gift = noChangeList.get(i);
			sn = saleContractGiftBufferCacl.createDetailField(sn, n, summaryBoard, gift);
			n++;
		}
	 ...
	 * @return 返回的最后序号
	 */
	public short createDetailField(short sn, int n, MobileBoardBuffered board, VehicleSaleContractGiftsVary gift) {
		String abortStatusStr = "";
		if (STATUS_NEW == gift.getAbortStatus()) {
			abortStatusStr = "新增";
		} else if (STATUS_MODIFY == gift.getAbortStatus()) {
			abortStatusStr = "修改";
		} else if (STATUS_ABORT == gift.getAbortStatus()) {
			abortStatusStr = "终止";
		}
		String title = String.format("%s %s", abortStatusStr, gift.getItemName());
		if (vehicleAbortStatus == STATUS_NEW) {
			title = String.format("%s", gift.getItemName());// 新增不用显示状态
		}
		...
		Property abortComment = getAbortComment(gift, gift.getAbortStatus());
	...
	// 金额
	private Property getAmount(VehicleSaleContractGiftsVary gift, Short abortStatus) {
	...
	// 是否赠送客户
	private Property getGiveFlag(VehicleSaleContractGiftsVary gift, Short abortStatus) {
		Property p = new Property();
		p.setLabel("是否赠送客户");
		String giveFlag = "否";
	...
	// 终止原因
	private Property getAbortComment(VehicleSaleContractGiftsVary gift, short status) {
		Property p = new Property();
		p.setLabel("终止原因");
		if (StringUtils.isEmpty(gift.getAbortComment())) {
			return null;
		}
		p.setValue(gift.getAbortComment());
		p.setShownOnSlaveBoard(true);
		p.setValueColour(Color.BLACK.getCode());
		return p;
	}
